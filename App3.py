            ...
            if results.multi_hand_landmarks:
                ...
                if fingerList[1] and fingerList[2]:
                    ...
                    if countCube == 0:
                        ...
                        countCube = 1
                        # Create array of square vertices
                        square_pts = np.array([[xstart_square, ystart_square], [x2, y2], 
                                            [x3, y3], [x4, y4]], np.int32)
                        square_pts = square_pts.reshape((-1, 1, 2))
                        
                        # Draw the square on all canvases
                        cv2.polylines(img, [square_pts], True, drawColor, brushThickness)
                        cv2.polylines(canvas, [square_pts], True, drawColor, brushThickness)
                        cv2.polylines(canvasBlack, [square_pts], True, drawColor, brushThickness)
                        countSquare = 1
