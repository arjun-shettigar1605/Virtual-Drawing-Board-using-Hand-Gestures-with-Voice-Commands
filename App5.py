                    ...
                    try:
                        ...
                        for shape_phrase, shape_tool in self.shape_commands.items():
                            if shape_phrase in command:
                                ...
                                break
                            elif shape_tool.lower() in command:
                                self.command_queue.put(f"draw {shape_tool}")
                                break
...
cap.set(4, 720)   # (4)height2q
cap.set(10, 150)  # brightness=150%

def generate_frames():
    
    global pending_shape_command
    pending_shape_command = None
    # Canvas
        ...
        while cap.isOpened():
                ...
                try:
                    ...
                    if command:
                        if command == "screenshot":
                            ...
                            selectedTool = 'Eraser'
                            drawColor = (0, 0, 0)
                            command_display = "Eraser selected"
                            last_command_time = time.time()
                        
                        elif command == "pencil" or command == "cancel":
                            tool = "Draw"
                            selectedTool = 'Draw'
                            ...
                            pending_shape_command = shape
                            command_display = f"Ready to draw {shape}" 
                            last_command_time = time.time()
                            print(f"Hand landmarks detected: {results.multi_hand_landmarks is not None}")
                            if results.multi_hand_landmarks:
                                print(f"Landmark length: {len(landMark)}")
                                print(f"Index finger position: {landMark[8][1:] if len(landMark) > 8 else 'Not available'}")
                            
                            if results.multi_hand_landmarks and len(landMark) >= 9:
                                xi, yi = landMark[8][1:]
                                print(f"Setting up {shape} drawing at position ({xi}, {yi})")
                                ...
                                if shape =="Circle":
                                    ...
                                    command_display = f"Drawing {shape}"
                                    last_command_time = time.time()
                                    print(f"Rectangle tool selected, starting at ({xstart_rect}, {ystart_rect})")
            ...
            if results.multi_hand_landmarks:
                ...
                if pending_shape_command and len(landMark) >= 9:
                    ...
                    pending_shape_command = None  # Reset pending command after executing
                ...
                if fingerList[1] and fingerList[2]: # 2 fingers up(Selection mode)
                    ...
                    if countSquare == 0:
                        ...
                        side_length = max(abs(xlast_square - xstart_square), abs(ylast_square - ystart_square))
                        
                        # Determine the end coordinates while maintaining square proportions
                        x_end = xstart_square + side_length if xlast_square > xstart_square else xstart_square - side_length
                        y_end = ystart_square + side_length if ylast_square > ystart_square else ystart_square - side_length
                        
                        # Draw the final square
                        cv2.rectangle(img, (xstart_square, ystart_square), (x_end, y_end), drawColor, brushThickness)
                        cv2.rectangle(canvas, (xstart_square, ystart_square), (x_end, y_end), drawColor, brushThickness)
                        cv2.rectangle(canvasBlack, (xstart_square, ystart_square), (x_end, y_end), drawColor, brushThickness)
                        countSquare = 1
                    
                    # Cube finishing
                    if countCube == 0:
                        # Calculate dimension
                        dim = abs(xlast_cube - xstart_cube)
                ...
                if fingerList[1] and fingerList[2] == 0:
                    ...
                    # For Square
                        ...
                        # Draw preview square
                        cv2.rectangle(img, (xstart_square, ystart_square), (x_end, y_end), drawColor, brushThickness)
                        xlast_square, ylast_square = x_end, y_end
